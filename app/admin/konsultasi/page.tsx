...
export default function ConsultationPage() {
  ...
  return (
    <div className="h-[calc(100vh-120px)] md:h-[calc(100vh-160px)] flex bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      {/* Left Column: Chat List */}
      <div className={`w-full md:w-80 border-r border-slate-100 flex flex-col ${!showListOnMobile ? "hidden md:flex" : "flex"}`}>
        <div className="p-4 border-b border-slate-100">
           <h2 className="text-lg font-bold text-slate-800 mb-4 md:hidden">Konsultasi</h2>
        ...
        <div className="flex-1 overflow-y-auto">
           {chatList.map((chat) => (
             <div 
                ...
                className={`p-4 flex gap-3 cursor-pointer hover:bg-slate-50 transition-colors ${activeChat === chat.id ? "bg-indigo-50/50 md:border-r-4 md:border-indigo-600" : ""}`}
             >
      ...
      <div className={`flex-1 flex flex-col bg-slate-50/30 ${showListOnMobile ? "hidden md:flex" : "flex"}`}>
        {/* Chat Header */}
        <div className="p-3 md:p-4 bg-white border-b border-slate-100 flex items-center justify-between">
           <div className="flex items-center gap-2 md:gap-3">
              ...
              <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-slate-100 overflow-hidden shrink-0">
                 <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${chatList.find(c => c.id === activeChat)?.name || 'User'}`} alt="" />
              </div>
              <div className="min-w-0">
                 <p className="text-sm font-bold text-slate-800 truncate">{chatList.find(c => c.id === activeChat)?.name}</p>
           ...
           <div className="flex items-center gap-0.5 md:gap-1">
              <button className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"><Phone size={18} className="md:w-5 md:h-5" /></button>
              <button className="hidden sm:block p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"><Video size={18} className="md:w-5 md:h-5" /></button>
              <button className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"><MoreVertical size={18} className="md:w-5 md:h-5" /></button>
           </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
           ...
           {messages.map((msg) => (
             <div key={msg.id} className={`flex ${msg.sender === 'doctor' ? 'justify-end' : 'justify-start'}`}>
                <div className={`group flex flex-col ${msg.sender === 'doctor' ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[70%]`}>
                   <div className={`px-4 py-2.5 md:py-3 rounded-2xl text-sm shadow-sm ${
                      msg.sender === 'doctor' 
        ...
        <div className="p-3 md:p-4 bg-white border-t border-slate-100">
           <div className="flex items-center gap-2 md:gap-3">
              <button className="p-2 text-slate-400 hover:text-indigo-600 transition-colors shrink-0"><Paperclip size={20} /></button>
              <div className="flex-1 relative">
                 <input 
                    type="text" 
                    placeholder="Tulis pesan..." 
                    className="w-full bg-slate-50 border border-slate-200 rounded-full py-2.5 md:py-3 px-4 md:px-5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all pr-10"
                    value={message}
              ...
              <button 
                onClick={() => setMessage("")}
                className="p-2.5 md:p-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/20 active:scale-95 shrink-0"
              >
